...
Aim: find a function $f(X)$ for predicting $X$ from some inputs $X$.
...
Marginally: $\text{argmin}_{\beta} \text{E}[(Y-X^T\beta)^2]$ gives $\beta=\text{E}[X X^T]^{-1}\text{E}[XY]$
...
Conditionally (known from before): if we assume that $(X,Y) \sim N_{p+1}(\mu,\Sigma)$ then we have seen that $\text{E}(Y\mid X)$ is linear in $X$ and $\text{Cov}(Y \mid X)$ is independent of $X$.
...
* $\hat{G} \in {\cal G}=\{1,\ldots,K\}$
* $L(G,\hat{G})$ is a $K\times K$ matrix where $K=\lvert G \rvert$, with elements $l_{jk}$ giving the price to pay to misclassify an observation with true class $g_j$ to class $g_k$. 
...
(ELS 7.2 and 7.4, and we are now back to a general loss function)
...
The following is Figure 7.1 from the ELS book. Explain what you see - in particular what is the red and blue lines and the bold lines. What can you conclude from the figure?

...
See [slides from guest lecturer Benjamin Dunn](slides link)
...
* Maximum likelihood estimation = maximize the likelihood of the data $\cal L(\beta_0,\beta; {\bf y}, {\bf X})$.
...
```{r,echo=TRUE}
ridgefit=glmnet(x=xss,y=ys,alpha=0,standardize=FALSE) # already standardized
plot(ridgefit,xvar="lambda",label=TRUE)

cv.ridge=cv.glmnet(x=xss,y=ys,alpha=0)
...
lassofit=glmnet(x=xss,y=ys,alpha=1,standardize=FALSE) # already standardized
plot(lassofit,xvar="lambda",label=TRUE)

cv.lasso=cv.glmnet(x=xss,y=ys,alpha=1)

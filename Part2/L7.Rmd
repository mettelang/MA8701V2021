...
b)  For each base learner $\Phi^l$ perform $V$-fold cross-validation to
    produce prediction.
...
b)  The metalearner is used to estimate the weights given to each base
    ...
    (Should probably also involve some link function, so that this may
...
a)  Use each of the $L$ base learners to produce a prediction
    ${\bf z}^*$, and
b)  feed this to the metalearnerfit to produce the final prediction
    $y^*$.
...
-   The oracle result require an *uniformly bounded loss function*.
    ...
    the training set is sufficient to allow for unbounded loss fuctions
    (Le Dell page 6).
...
Code is copied from [Guide to SuperLearner]() and the presentation
...
or preferably the distribution of the loss function for your problem.
...
The process is *second order stationary* if $\text{Var}(Y(t))=\sigma^2$

...
\NormalTok{tune_grid <-}\StringTok{ }\KeywordTok{expand.grid}\NormalTok{(}
  \DataTypeTok{mtry =}\NormalTok{ (}\DecValTok{1}\OperatorTok{:}\DecValTok{13}\NormalTok{))}
\CommentTok{#  ntree=seq(100,500,length=10)) # how to also include ntree? primary only mtry, how to define secondary?}
\NormalTok{tune_control <-}\StringTok{ }\NormalTok{caret}\OperatorTok{::}\KeywordTok{trainControl}\NormalTok{(}
  \DataTypeTok{method =} \StringTok{"oob"}\NormalTok{, }\CommentTok{# cross-validation #eller cv}
  \CommentTok{#number = 3, # with n folds }
  \DataTypeTok{verboseIter =} \OtherTok{FALSE}\NormalTok{, }\CommentTok{# no training log}
  \DataTypeTok{allowParallel =} \OtherTok{FALSE} \CommentTok{# FALSE for reproducible results }
\NormalTok{)}
\NormalTok{rf_tune <-}\StringTok{ }\NormalTok{caret}\OperatorTok{::}\KeywordTok{train}\NormalTok{(}
\NormalTok{  medv}\OperatorTok{~}\NormalTok{crim}\OperatorTok{+}\NormalTok{zn}\OperatorTok{+}\NormalTok{indus}\OperatorTok{+}\NormalTok{chas}\OperatorTok{+}\NormalTok{nox}\OperatorTok{+}\NormalTok{rm}\OperatorTok{+}\NormalTok{age}\OperatorTok{+}\NormalTok{dis}\OperatorTok{+}\NormalTok{rad}\OperatorTok{+}\NormalTok{tax}\OperatorTok{+}\NormalTok{ptratio}\OperatorTok{+}\NormalTok{black}\OperatorTok{+}\NormalTok{lstat, }
  \DataTypeTok{data=}\NormalTok{Boston,}
  \DataTypeTok{na.action=}\NormalTok{na.roughfix,}
  \DataTypeTok{trControl =}\NormalTok{ tune_control,}
  \DataTypeTok{tuneGrid =}\NormalTok{ tune_grid,}
  \DataTypeTok{method =} \StringTok{"rf"}\NormalTok{, }\CommentTok{# rf is randomForest, checked at #vhttp://topepo.github.io/caret/train-models-by-tag.html#Random_Forest}
  \DataTypeTok{verbose =} \OtherTok{TRUE}
\NormalTok{)}
\NormalTok{tuneplot <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x, }\DataTypeTok{probs =} \FloatTok{.90}\NormalTok{) \{}
  \KeywordTok{ggplot}\NormalTok{(x) }\OperatorTok{+}
\StringTok{    }\KeywordTok{coord_cartesian}\NormalTok{(}\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\KeywordTok{quantile}\NormalTok{(x}\OperatorTok{$}\NormalTok{results}\OperatorTok{$}\NormalTok{RMSE, }\DataTypeTok{probs =}\NormalTok{ probs), }\KeywordTok{min}\NormalTok{(x}\OperatorTok{$}\NormalTok{results}\OperatorTok{$}\NormalTok{RMSE))) }\OperatorTok{+}
\StringTok{    }\KeywordTok{theme_bw}\NormalTok{()}
\NormalTok{\}}
\KeywordTok{tuneplot}\NormalTok{(rf_tune)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rf_tune}\OperatorTok{$}\NormalTok{bestTune}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tree_rec <-}\StringTok{ }\KeywordTok{recipe}\NormalTok{(medv}\OperatorTok{~}\NormalTok{crim}\OperatorTok{+}\NormalTok{zn}\OperatorTok{+}\NormalTok{indus}\OperatorTok{+}\NormalTok{chas}\OperatorTok{+}\NormalTok{nox}\OperatorTok{+}\NormalTok{rm}\OperatorTok{+}\NormalTok{age}\OperatorTok{+}\NormalTok{dis}\OperatorTok{+}\NormalTok{rad}\OperatorTok{+}\NormalTok{tax}\OperatorTok{+}\NormalTok{ptratio}\OperatorTok{+}\NormalTok{black}\OperatorTok{+}\NormalTok{lstat, }\DataTypeTok{data =}\NormalTok{ Boston)}

\NormalTok{tune_spec <-}\StringTok{ }\KeywordTok{rand_forest}\NormalTok{( }\CommentTok{# parsnip interface to random forests models}
...
\CommentTok{#  min_n = tune()}
\NormalTok{) }\OperatorTok{%>%}
\CommentTok{#  set_mode("regression") %>%}
\CommentTok{#  set_engine("ranger",objective="reg:rmse") # errors with ranger}
\StringTok{  }\KeywordTok{set_engine}\NormalTok{(}\StringTok{"randomForest"}\NormalTok{) }\CommentTok{# randomforest ok}
...
\NormalTok{tune_param <-}\StringTok{ }\NormalTok{tune_spec}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{parameters}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{update}\NormalTok{(}\DataTypeTok{mtry=}\KeywordTok{mtry}\NormalTok{(}\KeywordTok{c}\NormalTok{(1L,13L)),}\DataTypeTok{trees=}\KeywordTok{trees}\NormalTok{(}\KeywordTok{c}\NormalTok{(100L,500L)))}

...
\begin{eqnarray*}
\hat{y}_k({\bf x}_i)=\frac{\exp({\bf w}_k^T {\bf h}_i+b_k)}{\sum_{s=1}^{K}\exp({\bf w}_s^T {\bf h}_i+b_s)}\\
J(\theta)=-\frac{1}{n}\sum_{i=1}^n\frac{1}{K} \sum_{k=1}^K (y_{ik}\ln({\hat{y}_k({\bf x}_i)})
\end{eqnarray*}
...
Assume not bias parameter (maybe center covariates and response)
...
In @WNvW we studied (briefly) the interpretation of the ridge penalty for multiple linear regression.
